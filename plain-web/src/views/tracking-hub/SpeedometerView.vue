  ...
  <div class="sensor-root">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <div class="stat-val">{{ distance.toFixed(2) }} km</div>
      </div>
      <div class="stat-card">
...
const speedOffset = computed(() => {
  const pct = Math.min(1, speed.value / MAX_SPEED)
  return arcLen * (1 - pct)
})
...
function onPosition(pos: GeolocationPosition) {
  ...
  speed.value = Math.max(0, kmh)
  if (speed.value > maxSpeed.value) maxSpeed.value = speed.value
  speedSum += speed.value
  speedCount++
  if (pos.coords.heading != null && !isNaN(pos.coords.heading)) {
    heading.value = headingToDir(pos.coords.heading)
  }
  if (lastPos) {
    distance.value += haversineKm(
